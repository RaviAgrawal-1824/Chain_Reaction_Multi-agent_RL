...
print("\n\nLet's play chain reaction game in 3x3\n")
game.print_grid()
for turn in range(turns):
    ...
    pos=mask_env(game)
    actions=available_action(game)
    # Qval_max = max(list(Q_table[new_pos].values()))
    row,col=max(Q_lookup[pos], key=Q_lookup[pos].get)
    
    if (row,col) in available_action(game):
        print(f"\nI will trigger the reaction at ({row+1}, {col+1})")
        obs,reward,done = game.step((row, col))
        game.print_grid()
    else:
        print('invalid action by agent')
        sys.exit()
    ...
    print('-'*50)
    print(f'Please play your turn:')
    row=int(input('Row:'))
    col=int(input('Col:'))
    if (row-1,col-1) in available_action(game):
        print(f"\nYou triggered the reaction at ({row}, {col})")
        obs,reward,done = game.step((row-1, col-1))
    else:
    ...
    turn=turn+1
    # print(f'\nWin value is {done} in the play count {turn}.')
